...
export function renderWelcomeEmail({template,...input}){
  ...
  try{
    return {html:renderWelcomeEmailTemplate(useEditable?candidate:FALLBACK_WELCOME_EMAIL_HTML,data),data,usedFallback:!useEditable};
  }catch(error){
    console.error('welcome email template render failed; using fallback',error);
    return {html:renderWelcomeEmailTemplate(FALLBACK_WELCOME_EMAIL_HTML,data),data,usedFallback:true};
  }
}
