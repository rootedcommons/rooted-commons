...
const expectedDate=value=>{const raw=String(value||'').trim();const match=raw.match(/^(\d{4}-\d{2}-\d{2})/);return match?match[1]:'';};
const ordinal=day=>{const n=Number(day)||0;const mod100=n%100;const suffix=mod100>=11&&mod100<=13?'th':n%10===1?'st':n%10===2?'nd':n%10===3?'rd':'th';return `${n}${suffix}`;};
function paymentSchedule(member){
  const frequency=unwrap(member['Contribution frequency'])==='Monthly'?'monthly':'weekly';
  const date=expectedDate(member['Regular payment overdue since']||member['Regular payment expected at']);
  if(!date)return {frequency,schedule:frequency==='monthly'?'its usual date':'its usual weekday'};
  const d=new Date(`${date}T12:00:00Z`);
  if(frequency==='monthly')return {frequency,schedule:`the ${ordinal(d.getUTCDate())}`};
  return {frequency,schedule:new Intl.DateTimeFormat('en-GB',{weekday:'long',timeZone:'Europe/London'}).format(d)};
}
...
export const FALLBACK_EMAILS={
  ...
  paymentOverdue:shell('Your regular payment hasn’t arrived',`<p>Hi {{first_name}},</p><div style="margin:20px 0;padding:18px 20px;background:#f8f2e9;border-left:4px solid #5a2d4d;"><p style="margin:0 0 12px;">Your regular {{payment_frequency}} payment usually arrives on <strong>{{payment_schedule}}</strong>, but it hasn’t arrived yet.</p><p style="margin:0;">Your <a href="https://rootedcommons.uk/faqs/#member-streaks" style="color:#5a2d4d;font-weight:bold;">Member streak</a> is frozen while we wait.</p></div><p>If you’re taking a break, <a href="https://rootedcommons.uk/faqs/#pauses" style="color:#5a2d4d;font-weight:bold;">let us know</a>. You can pause your regular commitment for up to 8 weeks in each calendar year without losing your streak.</p><p>If this was simply missed or delayed, please check your standing order. Your streak is protected for one calendar month from the missed payment date.</p>`),
...
export function lifecycleEmailData({member,settings={},pauseWeeksRemaining=0,pauseStart='',pauseEnd='',dashboardUrl='https://rootedcommons.uk/dashboard/',signinUrl='https://rootedcommons.uk/signin/'}){
  ...
  const payment=paymentSchedule(member);
  return {
...
function normaliseHeaderLogo(template,data){
  let html=String(template||'');
  if(data.header_logo_url){
    html=html.replace(/<strong\b[^>]*>\s*Rooted Commons\s*<\/strong>/i,'<img src="{{header_logo_url}}" width="150" alt="Rooted Commons" style="display:block;width:150px;max-width:80%;height:auto;border:0;margin:0 auto;">');
  }
...
function normaliseLifecycleTemplate(template,kind,data){
  let html=normaliseHeaderLogo(template,data);
  if(kind==='paymentOverdue'){
    const box='<div style="margin:20px 0;padding:18px 20px;background:#f8f2e9;border-left:4px solid #5a2d4d;"><p style="margin:0 0 12px;">Your regular {{payment_frequency}} payment usually arrives on <strong>{{payment_schedule}}</strong>, but it hasn’t arrived yet.</p><p style="margin:0;">Your <a href="https://rootedcommons.uk/faqs/#member-streaks" style="color:#5a2d4d;font-weight:bold;">Member streak</a> is frozen while we wait.</p></div>';
    html=html.replace(/<div\b[^>]*style=["'][^"']*border-left\s*:\s*4px solid #5a2d4d[^"']*["'][^>]*>[\s\S]*?<\/div>/i,box);
  }
  return html;
}
...
export function renderLifecycleEmail({kind,template,...input}){
  ...
  try{
    const source=normaliseLifecycleTemplate(candidate||fallback,kind,data);
    return {html:renderLifecycleTemplate(source,data),data,usedFallback:!candidate};
  }catch(error){
    console.error('membership lifecycle email template render failed; using fallback',{kind,error});
    return {html:renderLifecycleTemplate(normaliseLifecycleTemplate(fallback,kind,data),data),data,usedFallback:true};
  }
}
...
export function lifecycleEmailText(kind,data){
  const intros={
    ...
    paymentOverdue:`Your regular ${data.payment_frequency} payment usually arrives on ${data.payment_schedule}, but it hasn't arrived yet. Your Member streak is frozen while we wait. More about Member streaks: https://rootedcommons.uk/faqs/#member-streaks. If you're taking a break, let us know. Your streak is protected for one calendar month from the missed payment date.`,
